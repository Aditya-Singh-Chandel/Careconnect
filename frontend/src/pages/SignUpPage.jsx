...
function SignUpPage() {
  ...
  useEffect(() => {
    if (token) {
      ...
      setTimeout(() => {
        window.location.href = "/Careconnect/user/dashboard";
      }, 500);
    }
  }, [token]);
  ...
  const handleSignUp = async () => {
    ...
    const url = `${frontend_base_url}/Careconnect/login/`;
    // console.log(url);
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        ...
        emailRedirectTo: `${frontend_base_url}/Careconnect/login`, // Redirect URL after confirmation
      },
    });
